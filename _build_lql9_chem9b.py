#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""_build_lql9_chem9b.py — 化学九下单元页生成 (基座 lql_chem9_u1_interactive.html 延续 u 系列)
数据: _lql9_data/chem9_u{n}.json (n=8..12); 输出 lql_chem9_u{n}_interactive.html
"""
import json, os, re

REPO = '/home/administrator/xuci-jiancha'
BASE = os.path.join(REPO, 'lql_chem9_u1_interactive.html')

def build(n):
    data = json.load(open(os.path.join(REPO, '_lql9_data', f'chem9_u{n}.json'), encoding='utf-8'))
    h = open(BASE, encoding='utf-8').read()
    out = os.path.join(REPO, f'lql_chem9_u{n}_interactive.html')
    ...
    m = re.search(r'let curQ', h[ie:])  # 不限换行(chem基座 errors 后可能同行紧接)
    iend = ie + m.start() if m else h.find('</script>', ie)
    ...
    h = h.replace('lql_chem9_u1', f'lql_chem9_u{n}')
...
def main():
    for n in range(8, 13):
        p = os.path.join(REPO, '_lql9_data', f'chem9_u{n}.json')
        if not os.path.exists(p):
            print('⚠️ 缺数据 chem9_u' + str(n) + '.json')
            continue
        try:
            build(n)
        except Exception as e:
            print('❌ chem9_u' + str(n), e)
